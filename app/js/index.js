...
function filterStoriesBySection(data, section) {
    // Filter articles by section
    return data.results.filter((article) => article.section === section);
}

function sortStoriesByTitle(articles) {
...
function renderStories(section) {
    ...
    if (data) {
        // Filter articles by section
        const filteredArticles = filterStoriesBySection(data, section);

        // Sort filtered articles by title
        const sortedArticles = sortStoriesByTitle(filteredArticles);

        sortedArticles.forEach((story) => {
